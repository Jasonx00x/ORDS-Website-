...
function shopPage() {
  const productCards = merchProducts.map((product) => {
    const priceLabel = product.price ? `$${product.price}` : "Confirm";
    ...
    const mainMedia = activeMedia.type === "video"
      ? `<video class="merch-main-media" autoplay loop muted defaultMuted playsinline preload="metadata" poster="${activeMedia.poster || product.image}"><source src="${activeMedia.src}" type="video/mp4"></video>`
      : `<img class="merch-main-media gallery-main" src="${activeMedia.src}" alt="${escapeHtml(product.name)}">`;
    ...
    const mediaRail = activeVariant.media.length > 1 ? `<div class="media-rail">${activeVariant.media.map((media, index) => `<button class="media-thumb${index === 0 ? " active" : ""}" type="button" data-media-index="${index}" aria-label="${media.type === "video" ? "Play product video" : `View product photo ${index + 1}`}">${media.type === "video" ? `<span class="media-play">Play</span>` : `<img src="${media.src}" alt="">`}</button>`).join("")}</div>` : "";
    return `<article class="product-card merch-card${galleryClass} reveal" data-product-id="${product.id}" data-product-name="${escapeHtml(product.name)}" data-product-price="${product.price || ""}" data-product-price-label="${priceLabel}" data-product-sizes="${product.sizes.join("|")}" data-product-variants="${variantData}"><div class="gallery-frame">${mainMedia}<span class="stock-pill">${product.status}</span></div>${variantControls}${mediaRail}<div class="product-body"><div class="product-meta"><span>${product.category}</span><strong>${priceLabel}</strong></div><h3>${product.name}</h3><p>${product.description}</p><div class="size-chips">${product.sizes.map((size) => `<span>${size}</span>`).join("")}</div><button class="btn merch-add" type="button" data-add="${escapeHtml(productValue)}" data-product-id="${product.id}">Add to Request</button></div></article>`;
  }).join("");
